...
all_pkas = [];
new_AA = char(AA); % convert string of amino acids into a vector of individual amino acid letters

% looking through the sequence of amino acids for amino acids with pKRs and adding those pKRs to all_pkas[]
...
% creating list of all the pKas that must be considered for the amino acids in the sequence 
all_pkas = [0, all_pkas, pK1s_carbonyl(single_AA(1)), pK2s_amino_group(single_AA(end))];

% sorting pKas from lowest to highest
sorted_pkas = sort(all_pkas);

% iterating through all the pKas in the list for the pKa range at which the overall charge of the amino acid is 0, and then using that range to find the pI
